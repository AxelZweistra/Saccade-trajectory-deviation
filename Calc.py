...
for a in range(0,len(matches)):       
    ...
    Data = Data[Data[:,5] > 80]                         #latency above 80ms  
    NTrials[a,3] = len(Data[:,0]) / NTrials[a,0]        #selectionpercentage after latency 80 index 3
    Data = Data[Data[:,5] < 400]                        #latency under 400ms
    NTrials[a,4] = len(Data[:,0]) / NTrials[a,0]        #selectionpercentage after latency 400 index 4
    Data = Data[Data[:,6] < 1]                          #removing blink
    ...
    NTrials[a,5] = len(Data[:,0]) / NTrials[a,0]        #selectionpercentage after fix dist index 5
    Data = Data[Data[:,15] > 4]                         # amp above 4
    NTrials[a,6] = len(Data[:,0]) / NTrials[a,0]        #selectionpercentage after amp index 6
    ...
    SaccadeDeviation = np.zeros((len(Data[:,0]),1))
    ...
    for e in range(0,len(Data[:,0])):                            #for trials
        ...
        for z in range(0,len(Data[0,:])-121):                        #for y samples (n-y changes every datafile)
            if Data[e,20+z] == Data[e,13] and Data[e,120+z] == Data[e,14]:  
                break                                                #breaks if next sample is endsample
            ...
            try:
                ...
                print(str(matches[a][68:81]) + ' - Trial: ' + str(e+1) + ' - Sample: ' + str(z)) #prints the position of trials that raise an error
            else:
                fraction = (SideA**2 + SideB**2 - SideC**2) / (2 * SideA * SideB)
                OutputMatrix[e,z] = np.degrees(np.math.acos(fraction)) #sets outputmatrix to the fraction
        
        OutputMatrix[OutputMatrix == 0] = np.nan              #sets empty values to NaN ensuring they dont affect the means
        SaccadeDeviation[e] = np.nanmean(OutputMatrix[e,:])   #sets SaccadeDeviation to mean of all angles
        InitialDirection[e] = OutputMatrix[e,11]              #sets initialdirection to the angle after the first 10 datapoints
    ...
    DataAppend = np.stack((Participant[:,0],Phase[:,0],ConditionedColour[:,0],Data[:,4],Data[:,0],Data[:,2],SaccadeDeviation[:,0], InitialDirection[:,0]), axis=1)    
    OutPut = np.concatenate((OutPut,DataAppend), axis = 0)
    
np.savetxt('Datafile.csv', OutPut, delimiter = ',', fmt='%f' )   #saves output file as CSV
